...
impl RTCCertificate {
    ...
    pub fn from_key_pair(key_pair: KeyPair) -> Result<Self> {
        ...
        // `params.self_signed(&key_pair)` creates a self-signed certificate.
        // The algorithm is inferred from the provided `key_pair`.
        //let x509_cert = params.self_signed(&key_pair)?;

        // Convert the rcgen::Certificate (x509_cert) into your RTCCertificate type.
        RTCCertificate::from_params(cert.params().clone())
    }
